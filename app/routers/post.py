...
def get_post(db: Session = Depends(get_db), current_user: int= Depends(oauth.get_current_user), limit: int = 10, skip: int = 0, search: Optional[str]= "" ):
...
def get_one(id: int, db: Session = Depends(get_db), current_user: int= Depends(oauth.get_current_user)):
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int= Depends(oauth.get_current_user)):
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code= status.HTTP_403_FORBIDDEN, detail= f"Not authurized tomperform operation")
